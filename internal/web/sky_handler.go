...
func (s *Server) handleSky(w http.ResponseWriter, r *http.Request) {
	...
	points := make([]SkyPointDTO, 0, len(sights.Items))
	for _, s := range sights.Items {
		el := los.ElevationAngle(s.Unit, basis)
		az := los.AzimuthAngle(s.Unit, basis)
		used := false
		for _, f := range view.Visible {
			if f.SatID == s.SatID {
				used = true
				break
			}
		...
		points = append(points, SkyPointDTO{
			ID:           s.SatID,
			AzimuthDeg:   los.Degrees(az),
			ElevationDeg: los.Degrees(el),
			Used:         used,
		})
	}
	counts := view.Counts()
