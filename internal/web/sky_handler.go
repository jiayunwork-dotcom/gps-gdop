...
// handleSky serves POST /api/sky: it computes the topocentric direction
// of every satellite, so the page can draw a zenith chart from the
// backend-computed azimuth and elevation rather than hard-coding star
// positions. The same input rules as /api/dop apply: fewer than four
// satellites, a coincident satellite or an out-of-range mask is rejected
